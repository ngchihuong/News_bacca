...
export default function ProfileSettingsPage() {
  const { user, isAuthenticated, setUser, refetchAccount } = useAppContext();
  ...
  const [avatarPreview, setAvatarPreview] = useState<string | null>(
    user?.avatarUrl ? getFullAvatarUrl(user.avatarUrl) : null
  );
  ...
  if (isLoadingProfile) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-12 flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <p className="text-gray-600 dark:text-slate-400 font-medium">Đang tải thông tin hồ sơ của bạn...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {statusMessage && (
        ...
        >
          {statusMessage.type === "success" ? (
            <FaCheckCircle className="text-lg text-green-600 dark:text-green-400 flex-shrink-0" />
          ) : (
            <FaExclamationCircle className="text-lg text-red-600 dark:text-red-400 flex-shrink-0" />
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1">
          <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700/60 rounded-2xl p-6 shadow-sm flex flex-col items-center text-center">
            {/* Avatar container */}
            <div className="relative group">
              <div className="w-32 h-32 rounded-full ring-4 ring-orange-100 dark:ring-orange-950/60 overflow-hidden shadow-inner bg-gray-50 dark:bg-slate-700">
                <img
            ...
            <div className="mt-4">
              <h2 className="text-lg font-bold text-gray-900 dark:text-slate-100">
                {profile?.fullName || user?.name || profile?.username || user?.username}
              </h2>
              <p className="text-sm text-gray-500 dark:text-slate-400">@{profile?.username || user?.username}</p>
            </div>

            <div className="mt-3">
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-orange-100 dark:bg-orange-950/60 text-orange-800 dark:text-orange-300">
                {(profile?.role || user?.role)?.replace("ROLE_", "") || "USER"}
              </span>
            </div>

            <div className="mt-6 pt-6 border-t border-gray-100 dark:border-slate-700/60 w-full text-left text-xs text-gray-500 dark:text-slate-400 space-y-2">
            ...
            <button
              ...
              onClick={() => fileInputRef.current?.click()}
              disabled={isUploadingAvatar}
              className="mt-6 w-full py-2 px-3 border border-orange-200 dark:border-orange-800/80 text-orange-700 dark:text-orange-300 bg-orange-50 dark:bg-orange-950/40 hover:bg-orange-100 dark:hover:bg-orange-900/60 rounded-xl text-xs font-semibold transition disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {isUploadingAvatar ? (
                <>
        ...
        <div className="md:col-span-2">
          <form
            ...
            className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700/60 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6"
          >
            ...
            <div>
              ...
              <div className="relative rounded-xl">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-10 py-2.5 bg-gray-50 dark:bg-slate-900/60 border border-gray-200 dark:border-slate-700 rounded-xl text-sm text-gray-500 dark:text-slate-400 cursor-not-allowed"
                />
            ...
            <div>
              <label
                htmlFor="fullName"
                className="block text-xs font-semibold uppercase tracking-wider text-gray-700 dark:text-slate-200 mb-1.5"
              >
                Họ và tên
              </label>
              <div className="relative rounded-xl">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded-xl text-sm text-slate-900 dark:text-slate-100 placeholder:text-gray-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition"
                />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                <label
                  htmlFor="bio"
                  className="block text-xs font-semibold uppercase tracking-wider text-gray-700 dark:text-slate-200"
                >
                  Tiểu sử (Bio)
                </label>
                <span
                  className={`text-xs font-medium ${
                    bio.length > 500 ? "text-red-500 dark:text-red-400" : "text-gray-400 dark:text-slate-400"
                  }`}
                >
              ...
              <textarea
                ...
                className={`w-full p-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder:text-gray-400 dark:placeholder:text-slate-500 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:border-transparent transition ${
                  bio.length > 500
